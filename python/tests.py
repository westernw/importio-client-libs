...
'''
try:
	client = importio.importio(userId=str(uuid.uuid4()), apiKey=apikey)
	client.connect()
...
'''
try:
	client = importio.importio(userId=userguid, apiKey=str(uuid.uuid4()))
	client.connect()
...
client = importio.importio(userId=userguid, apiKey=apikey)
client.connect()
