 ...
 */
public class IO4Test extends TestHelper {
	...
	public void test() {
			...
			public void onMessage(Query query, QueryMessage message, Progress progress) {
				if (message.getType() == MessageType.MESSAGE) {
					...
					if (resultMessage.containsKey("errorType")) {
						if (resultMessage.get("errorType").equals("ConnectorNotFoundException")) {
							success = true;
						}
					}
				}
		...
		query.setConnectorGuids(Arrays.asList(UUID.randomUUID()));
